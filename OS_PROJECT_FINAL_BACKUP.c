...
int find_node(struct node **root, int count, char *line)
{
	...
	while(true) {
		if(temp->cmd_cnt == count) {
			...
			printf("find_node: node found\n");
			printf("find_node: line = %s\n", line);
			break;
		}
		else
			temp = temp->next;
	}
	printf("return value from find_node: %d\n",ret);
	return ret;
}
...
/*
		...
		for (int t=1;t<len;t++){
			temp_line[t-1] = line[t];
			printf("temp_line[%d] = %c\n",(t-1),temp_line[t-1]);
		}
	
		   if(0 == strcmp(temp_line,"!")) {
			printf("request to run last run command\n");
			if(root != NULL){
				if(!(find_node(&root, count,line))) {
					printf("command with number %d not found\n", count);
				} else {
			...
			return_val = find_node(&root, atoi(temp_line), line);
			if (!return_val){
