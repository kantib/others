...
public class MasterBot {
    ...
    public static void main(String[] args) {
        ...
        while(true) {
            ...
            //    System.out.println("wordArray["+j+"] = "+wordArray[j]);
            ...
            if(command.equals("connect")) {
                if(wordCount >=5 && wordCount <=7) {
                    if(wordCount == 7) {
                        /*last 3 optional "numOfConnections","keepalive","url" options are given*/
                        command_buf = wordArray[0]+" "+wordArray[2]+" "+wordArray[3]+" "+wordArray[4]+" "+wordArray[5]+" "+wordArray[6];
                        //System.out.println("command1 :: "+command_buf);
                    }
                    if(wordCount == 6) {
                        /*combination of last 3 optional arguments is given*/

                        /*Case : "keepalive", "url" given ==> "numOfConnections" missing*/
                        if(wordArray[4].equals("keepalive")) {
                            num_connections ="1";
                            command_buf = wordArray[0]+" "+wordArray[2]+" "+wordArray[3]+" "+num_connections+" "+wordArray[4]+" "+wordArray[5];
                            //System.out.println("command2 :: "+command_buf);
                        } else {
                            /*Case : "numOfConnections","keepalive" given  OR "numOfConnections","url" given*/
                            command_buf = wordArray[0]+" "+wordArray[2]+" "+wordArray[3]+" "+" "+wordArray[4]+" "+wordArray[5];
                            //System.out.println("command3 :: "+command_buf);
                        }
                    }
                    if(wordCount == 5) {
                        /*Any one among last 3 optional arguments ("numOfconnections","keepalive","url") is given*/
                        if(wordArray[4].equals("keepalive") || wordArray[4].startsWith("url")) {
                            num_connections = "1";
                            command_buf = wordArray[0]+" "+wordArray[2]+" "+wordArray[3]+" "+num_connections+" "+wordArray[4];
                            //System.out.println("command4 :: "+command_buf);
                        } else {
                            command_buf = wordArray[0]+" "+wordArray[2]+" "+wordArray[3]+" "+" "+wordArray[4];
                            //System.out.println("command5 :: "+command_buf);
                        }
			...
			//System.out.println("command6 :: "+command_buf);
		} else {
                    System.out.println("Usage: " + command + " <SlaveHostName>|<SlaveIP>|all "+
                            "<TargetHostName>|<TargetIP>  <TargetPort> [<NumberOfConnections>][keepalive][url=path-to-be-provided-to-the-webserver]");
                    continue;
                }
            ...
            for(int i=0; i < slavelist.size(); i++){
                ...
                if (targetSlave.equals("all") || (hostIP.equals(targetSlave) || hostName.equals(targetSlave))) {
                    ...
                    try {
                        ...
                        //We got IO exception,probably slave is already disconnected
                        so.inactive=true;
                    }
                }
            }

            //remove the array elements for any inactive slaves
            ListIterator<SlaveObj> it = slavelist.listIterator();
            while(it.hasNext()) {
                SlaveObj so = it.next();
                if (so.inactive) {
                    it.remove();
